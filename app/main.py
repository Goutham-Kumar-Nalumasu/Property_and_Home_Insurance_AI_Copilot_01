...
app = FastAPI(title="HomeShield Property Insurance Copilot API", version="1.0.0")

@app.get("/")
def root():
    return {"message": "HomeShield Property Insurance Copilot API is running.", "docs": "/docs"}

@app.post("/ingest")
def ingest_documents():
    chunks = build_chunks()
    rag_retriever.reload()
    return {"message": "Documents ingested successfully.", "chunks_created": len(chunks)}

@app.post("/chat", response_model=ChatResponse)
def chat(request: ChatRequest):
    return run_agent(request.session_id, request.message)
